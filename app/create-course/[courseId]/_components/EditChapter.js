...
const EditChapter = ({ course, index, refreshData }) => {
  const [name, setName] = useState(
    course.courseOutput.chapters[index].courseName
  );
    ...
    <AlertDialog>
      <AlertDialogTrigger>
        <h3 className="text-blue-700 text-lg cursor-pointer">
          <BiSolidEdit className="text-blue-700 text-2xl" />
        </h3>
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            <div className="mt-4">
              <label className="block w-full text-left text-sm font-medium text-gray-700 mb-1">
                Chapter Title
              </label>
              <input
